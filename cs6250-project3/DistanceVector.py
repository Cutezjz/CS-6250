...
class DistanceVector(Node):
    ...
    def process_BF(self):
        ...
        for msg in self.messages:
            ...
            for node in msg.keys():
                if(node!=self.name):
                    if(node not in self.dist.keys()): #Distance to other nodes via neighbhor
                        self.dist[node]=msg[self.name]+msg[node]
                        update=True
                    else: #Update distance if an alternative route is shorter
                        if(msg[self.name]+msg[node]<self.dist[node]):
                            self.dist[node]=msg[self.name]+msg[node]  
                            update=True
    ...
    def log_distances(self):
        ...
        switch = self.name
        nodes=sorted(self.dist.keys())
        logstring=[]
        
        for x in nodes:
            string=x+str(self.dist[x])
            logstring.append(string)


        toAdd=",".join(logstring)
        add_entry(self.name,toAdd)
